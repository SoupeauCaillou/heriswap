...
if [ $# = 0 ]; then 
	echo "This will generate apk bin/Heriswap-signed.apk which can be uploaded on google console https://play.google.com/apps/publish/ .
...
#replace log level
sed -i "s/SacActivity.LogLevel = SacActivity.*/SacActivity.LogLevel = SacActivity.F;/" platforms/android/src/net/damsy/soupeaucaillou/heriswap/HeriswapActivity.java
...
git checkout AndroidManifest.xml platforms/android/src/net/damsy/soupeaucaillou/heriswap/HeriswapActivity.java
