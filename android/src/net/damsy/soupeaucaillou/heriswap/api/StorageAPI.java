...
public class StorageAPI {
	...
	static public boolean soundEnable(boolean switchIt) {
		SharedPreferences preferences = SacJNILib.activity
				.getSharedPreferences(HeriswapActivity.HERISWAP_SHARED_PREF, 0);
	...
	static private float bestLocalTimeForModeAndDifficulty(SQLiteDatabase db,
			final int mode, final int difficulty) {
		Cursor cursor = null;
		try {
			cursor = db.query("score", new String[] { "time" }, "mode='"
					+ (mode + 1) + "' and difficulty='" + difficulty + "'",
	...
	static private int bestLocalScoreForModeAndDifficulty(SQLiteDatabase db,
			final int mode, final int difficulty) {
		Cursor cursor = null;
		try {
			cursor = db.query("score", new String[] { "points" }, "mode='"
					+ (mode + 1) + "' and difficulty='" + difficulty + "'",
					null, null, null, "points desc");
			if (cursor.getCount() > 0) {
				cursor.moveToFirst();
				return cursor.getInt(0);
			}
			...
			for (int _diff = 0; _diff < 3; _diff++) {
				...
				SwarmLeaderboard.GotLeaderboardCB callback = new SwarmLeaderboard.GotLeaderboardCB() {
					...
					public void gotLeaderboard(
							final SwarmLeaderboard leaderboard) {
						if (leaderboard != null) {
	...
	static public void submitScore(final int mode, final int difficulty,
			final int points, final int level, final float time,
			final String name) {
		SQLiteDatabase db = HeriswapActivity.scoreOpenHelper
				.getWritableDatabase();
